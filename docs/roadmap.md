...
| Operational logging | `done` | 提供最小化本地运维日志，不记录 prompt 正文 | 默认写入 `log/dev/yy-mm/yy-mm-dd.log`，覆盖 request/session/thread/model/latency/status 等元数据 | `src/observability/bridge-logger.ts`, `src/observability/file-log-sink.ts`, `src/observability/request-logging.ts` |
